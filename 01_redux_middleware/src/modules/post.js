...
//thunk를 만들어 요청 상태에 따라 액션을 디스패치하세요. 이번에 만들 thunk는 나중에 컴포넌트를 불러와 사용해야 하므로 export를 입력하여 내보내세요.
export const getPost = (postId) => dispatch => {
    //먼저 요청이 시작했다는 것을 알립니다.
    dispatch(getPostPending());

    //요청을 시작합니다. 여기에서 만든 promise를 return해야 나중에 컴포넌트에서 호출할 때 getPost().then(...)을 할 수 있습니다.
...
// 리듀서의 초기 상태를 정의하고 handleActions를 사용하여 리듀서 함수를 구현
const initialState = {
    pending: false,
    error: false,
    data: {
